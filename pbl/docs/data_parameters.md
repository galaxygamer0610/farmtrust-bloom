...
- **Net Profit**: `Numeric` (Optional input, auto-computed if blank)
- **Loan Amount**: `Numeric` (Outstanding loan in ₹ thousands, >= 0)
- **Debt-to-Equity Ratio**: `Numeric` (Optional input, auto-computed if blank)

### Agricultural Operational Fields (Used for ML & Subsidies):
...
- **Yield Amount**: `Numeric` (Recent yield performance index, scale 1-10)
